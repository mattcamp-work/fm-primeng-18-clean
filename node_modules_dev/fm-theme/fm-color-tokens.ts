...
function makeLightnessRamp(_hueSatValue:string,_invert:boolean = false) {
  const _ramp:any = {};

  levels.forEach((_level) => {

     let  _lightness: number = 100 - parseFloat(_level);

    if(_invert == true) {
     _lightness = parseFloat(_level);
    } 
...
let darkSurface = makeLightnessRamp(hueSat.slate,true);
